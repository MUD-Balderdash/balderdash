...
MPEDIT(mpedit_show)
{
    ...
    if (!IS_NULLSTR(buf) && !str_prefix(buf, "formatted"))
    {
        sprintf(buf,
           "Vnum:       [%d]\n\r"
           "Code:\n\r", pMcode->vnum);
...
OPEDIT(opedit_show)
{
    ...
    if (!IS_NULLSTR(buf) && !str_prefix(buf, "formatted"))
    {
        ...
        format_code_output(ch, pOcode->code);
    }
    else
    {
        sprintf(buf,
...
RPEDIT(rpedit_show)
{
    ...
    if (!IS_NULLSTR(buf) && !str_prefix(buf, "formatted"))
    {
        ...
        format_code_output(ch, pRcode->code);
    }
    else
    {
        sprintf(buf,
...
#define str_replace(seed, apple, tree) string_replace(buf, seed, apple)
