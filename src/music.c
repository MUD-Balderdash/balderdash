...
void do_play(CHAR_DATA *ch, char *argument)
{
	...
	for (i = 0; i < MAX_SONGS; i++)
	{
		...
		sprintf(buf, "[%2d] %-39s %-39s\n\r",
			i + 1,  song_table[i].group, song_table[i].name);
	    else if (!artist && (!match 
				 || !str_prefix(argument, song_table[i].name)))
		sprintf(buf, "[%2d] %-35s ", i + 1, song_table[i].name);
	    else
		continue;
	    add_buf(buffer, buf);
	    if (!artist && ++col % 2 == 0)
	...
	if (i == song + 1 || !str_prefix(argument, song_table[song].name))
	    break;
    }
