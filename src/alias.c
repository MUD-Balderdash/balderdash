 ...
 */
/*
 * XXX Unfinished tale... XXX
 */

/*
#define NUM_TOKENS       9

void smart_aliases(DESCRIPTOR_DATA *d, char *subst, char *target)
{
    char *command, *arg[NUM_TOKENS];
	...
	for (i = 1; i < 10; i++)
	    ...
	    {
		sprintf(tmp, "$%d", i);
		string_replace(command, tmp, arg[i]);
	    }
	string_replace(command, "$*", buf);
...
/*    for (alias = 0; cmd_table[alias].name[0] != '\0'; alias++)
	...
	if (!str_prefix(ch->pcdata->alias[alias], argument))
	{
		...
		/* XXX smart_aliases(d, buf, point); XXX */
		break;
	    }
	}
    }
