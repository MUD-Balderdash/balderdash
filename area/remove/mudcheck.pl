...
$path		=	"/usr/local/mud/area";
$process	=	"rom";
$exec		=	"mud.server";
$pidfile	=	"$path/$process.$port";
$maintainpid	=	"$path/maintains.pid";
...
# Check if maintain script is working
if (($pid = &read_pid($maintainpid)) && kill 0,$pid)
{
  exit 0;
}  
...
if ($cc == 0) {
	...
	print ("Connection to $server on port $port failed!\n");
...
sub time_out
{
  print ("Connection to $server on port $port timed out after $timeOut seconds!\n");
  $time = (scalar localtime);
  print ("Attempting to restart the mud on $time...\n");
  # Restart the mud						#
  &restart_mud;
		...
		if (($pid = &read_pid($pidfile)))
		{
		  ...
		  if (kill TERM, $pid)
		  {
		    ...
		    return;
                ...
                system("killall -KILL $process");
                  
		open(FH, "ps ax | grep $exec | grep -v grep |");
		$out = <FH>;
		close(FH);
